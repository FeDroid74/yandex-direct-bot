...
- Feed registration, campaign creation, editing, moderation, launch and stopping the old campaign are separate owner-approved cards. Creation stays DRAFT/OFF without ads moderation; this is not SUSPENDED. Moderation of new OFF/DRAFT campaigns is blocked until safe no-launch handling is verified. Never bypass cards with legacy campaign routes or shell approvals.
...
- For the specifically verified empty draft after suspend error 8300, use a new product_complete_create card with the existing campaign_id and source_proposal_id. It never repeats campaigns.add and still requires a real owner click. See docs/PRODUCT_CAMPAIGNS.md.
